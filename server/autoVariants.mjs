...
function getBaseName(name) {
  return name
    .replace(/\s*[\(\[].*?[\)\]]/g, '')
    .replace(/\s*-\s*pack of \d+/gi, '')
    .replace(/\s*\d+\s*(kg|gm|g|ml|l|ltr|litre|pc|pcs|pieces?)\s*$/gi, '')
    .replace(/\s*x\s*\d+/gi, '')
    .trim();
}
...
async function run() {
  ...
  for (const [slug, items] of Object.entries(groups)) {
    ...
    // Fix: update ALL matching products regardless of current variantGroup value
