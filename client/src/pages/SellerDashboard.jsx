...
const SellerDashboard = () => {
    ...
    const allOrdersSorted = [...allOrders].sort((a, b) =>
    ...
    if (loading) {
        return (
            <div className='p-4 bg-orange-50 min-h-screen max-w-4xl mx-auto flex items-center justify-center'>
    ...
    return (
        <div className='p-4 bg-orange-50 min-h-screen max-w-4xl mx-auto'>
            ...
            <div className='flex gap-2 mb-6 bg-white rounded-2xl p-1 shadow-sm border border-slate-100'>
                <button onClick={() => setActiveTab('packing')} className={`flex-1 py-2.5 rounded-xl font-black text-sm transition-all ${activeTab === 'packing' ? 'bg-orange-500 text-white shadow' : 'text-slate-500'}`}>📦 Packing</button>
                <button onClick={() => setActiveTab('history')} className={`flex-1 py-2.5 rounded-xl font-black text-sm transition-all ${activeTab === 'history' ? 'bg-slate-800 text-white shadow' : 'text-slate-500'}`}>🧾 History</button>
                <button onClick={() => setActiveTab('earnings')} className={`flex-1 py-2.5 rounded-xl font-black text-sm transition-all ${activeTab === 'earnings' ? 'bg-green-600 text-white shadow' : 'text-slate-500'}`}>💰 Earnings</button>
            </div>
                ...
                <div className='grid gap-4'>
                    {packingOrders.length === 0 ? (
                        <div className='bg-white rounded-3xl p-12 text-center border-2 border-dashed border-orange-200'>
                            ...
                            <button onClick={fetchOrders} className='mt-4 bg-orange-100 text-orange-600 font-black text-sm px-5 py-2 rounded-full hover:bg-orange-200 transition-all'>Check for new orders</button>
                        </div>
                    ) : (
                        packingOrders.map(order => (
                            <div key={order._id} className='bg-white p-6 rounded-3xl shadow-sm border-2 border-orange-100'>
                                ...
                                <button onClick={() => markAsReady(order.orderId)} className='w-full bg-orange-500 text-white py-3 rounded-2xl font-black hover:bg-orange-600 transition-all active:scale-95'>PACKED & READY FOR RIDER</button>
                            </div>
                        ))
                    )}
                </div>
                        ...
                        allOrdersSorted.map(order => (
                            <div key={order._id} className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'>
                                ...
                                <div className='mb-2'>
                                    ...
                                    {(order.cartItems || []).length > 3 && <p className='text-xs text-slate-400'>+{order.cartItems.length - 3} more items</p>}
                                </div>
                                <div className='flex justify-between items-center pt-2 border-t border-slate-50'>
                                    ...
                                    <span className='text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-lg'>{order.payment_status === 'CASH ON DELIVERY' ? '💵 COD' : '✅ Paid'}</span>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div className='flex flex-col gap-4'>
                    <div className='flex gap-2 flex-wrap'>
                        {[{ key: 'today', label: 'Today' }, { key: 'week', label: 'This Week' }, { key: 'month', label: 'This Month' }, { key: 'all', label: 'All Time' }].map(f => (
                            <button key={f.key} onClick={() => setEarningFilter(f.key)} className={`px-4 py-2 rounded-full text-xs font-black transition-all border ${earningFilter === f.key ? 'bg-green-600 text-white border-green-600' : 'bg-white text-slate-600 border-slate-200'}`}>{f.label}</button>
                        ))}
                    </div>
                    <div className='bg-gradient-to-br from-green-600 to-emerald-700 rounded-2xl p-5 text-white shadow-lg'>
                        ...
                        <div className='flex gap-4 mt-3 border-t border-green-500 pt-3'>
                            ...
                            <div><p className='text-[10px] text-green-200 uppercase font-bold'>Delivery</p><p className='text-sm font-bold text-red-300'>-₹{totalDeliveryFees.toLocaleString()}</p></div>
                    ...
                    <div className='grid grid-cols-2 gap-3'>
                        <div className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'><p className='text-[10px] font-black text-slate-400 uppercase'>Gross Revenue</p><p className='text-2xl font-black text-slate-600 mt-1'>₹{totalGross.toLocaleString()}</p><p className='text-[10px] text-slate-400 mt-1'>Before delivery deduction</p></div>
                        <div className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'><p className='text-[10px] font-black text-slate-400 uppercase'>Delivery Deducted</p><p className='text-2xl font-black text-red-400 mt-1'>₹{totalDeliveryFees.toLocaleString()}</p><p className='text-[10px] text-slate-400 mt-1'>Paid to delivery boy</p></div>
                        <div className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'><p className='text-[10px] font-black text-slate-400 uppercase'>Net Earning</p><p className='text-2xl font-black text-emerald-600 mt-1'>₹{totalNet.toLocaleString()}</p><p className='text-[10px] text-slate-400 mt-1'>Your actual income</p></div>
                        <div className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'><p className='text-[10px] font-black text-slate-400 uppercase'>Avg Per Order</p><p className='text-2xl font-black text-blue-600 mt-1'>₹{avgNet}</p><p className='text-[10px] text-slate-400 mt-1'>Net avg value</p></div>
                    </div>
                    <div className='bg-white rounded-2xl p-4 shadow-sm border border-slate-100'>
                        ...
                        {Object.keys(earningsByDate).length === 0 ? (
                            ...
                            <div className='flex flex-col gap-2'>
                                {Object.entries(earningsByDate).sort((a, b) => new Date(b[0]) - new Date(a[0])).map(([date, data]) => (
                                    <div key={date} className='flex justify-between items-center py-2 border-b border-slate-50 last:border-0'>
                                        <div><p className='font-bold text-slate-700 text-sm'>{date}</p><p className='text-[10px] text-slate-400'>{data.count} order{data.count > 1 ? 's' : ''} · -₹{data.delivery} delivery</p></div>
                                        <div className='text-right'><p className='font-black text-emerald-600'>₹{data.net.toLocaleString()}</p><p className='text-[10px] text-slate-400 line-through'>₹{data.gross}</p></div>
                                    </div>
                                ))}
                            </div>
                        )}
