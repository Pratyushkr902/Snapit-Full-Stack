...
export default function RestaurantAdminPage() {
  ...
  const [tab, setTab] = useState('restaurants') // restaurants | menu | addResto | addItem
      ...
      <div className='bg-slate-900 border-b border-slate-800 flex overflow-x-auto'>
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-3 text-xs font-black whitespace-nowrap border-b-2 transition-all ${tab===t.key ? 'border-orange-500 text-orange-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
            {t.label}
          </button>
        ))}
      </div>
                ...
                <div className='flex gap-3 p-4'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-2 mb-0.5'>
                      ...
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${r.isOpen ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{r.isOpen ? 'OPEN' : 'CLOSED'}</span>
                    </div>
            ...
            {Object.entries(menuItems.reduce((acc, item) => {
              ...
              <div key={cat} className='mb-5'>
                ...
                <div className='flex flex-col gap-2'>
                  {items.map(item => (
                    <div key={item._id} className={`bg-slate-900 border rounded-xl p-3 flex gap-3 ${item.isAvailable ? 'border-slate-800' : 'border-slate-700 opacity-50'}`}>
