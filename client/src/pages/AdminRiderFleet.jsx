...
import { FaMotorcycle, FaPhone, FaMapMarkerAlt, FaClock, FaCheckCircle, FaMoneyBillWave, FaCompass, FaLayerGroup, FaExpand, FaCompress, FaCrosshairs, FaWhatsapp, FaUserPlus, FaCheck, FaTimes, FaCopy, FaBicycle, FaBolt } from 'react-icons/fa'
...
const AdminRiderFleet = () => {
  ...
  const fetchApplications = useCallback(async (statusFilter = appFilter) => {
    try {
      setLoadingApps(true)
      const res = await Axios({
        ...SummaryApi.getAllRiderApplications,
  ...
  const handleApprove = async (appId, appName) => {
    try {
      ...
      const res = await Axios({
        ...SummaryApi.approveRiderApplication,
        data: { applicationId: appId }
      })
      if (res.data?.success) {
        toast.success(res.data.message || `Rider ${appName} approved & activated!`)
        fetchApplications(appFilter)
        fetchFleet(true)
      } else {
        toast.error(res.data?.message || 'Failed to approve application')
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Error approving application')
    } finally {
  ...
  const handleReject = async (appId, appName) => {
    if (!window.confirm(`Are you sure you want to reject the application from ${appName}?`)) return
    try {
      setActionLoadingId(appId)
      const res = await Axios({
        ...SummaryApi.rejectRiderApplication,
        data: { applicationId: appId, adminNotes: 'Rejected by admin' }
      })
      if (res.data?.success) {
        toast.success(`Application for ${appName} marked as rejected.`)
        fetchApplications(appFilter)
      } else {
  ...
  return (
    <div className='min-h-screen bg-slate-950 text-white pb-12 w-full max-w-full overflow-x-hidden'>
      ...
      <div className='max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6 w-full max-w-full'>
        ...
        {/* ── PRIMARY TAB SWITCHER: FLEET vs APPLICATIONS ── */}
        <div className='flex flex-wrap items-center justify-between gap-3 mb-6 border-b border-slate-800 pb-4'>
          <div className='flex items-center gap-2'>
            ...
            >
              ...
              <span>Live Fleet ({totalRiders})</span>
            </button>
          ...
          <div className='flex items-center gap-2'>
            <button
              ...
              className='px-3.5 py-2 rounded-2xl bg-slate-900 hover:bg-slate-800 border border-slate-700/80 text-xs text-slate-300 hover:text-white font-bold transition flex items-center gap-1.5 shadow-sm'
              title='Copy Public Onboarding URL to send via WhatsApp'
            >
              <FaCopy className='text-emerald-400 text-xs' />
              <span>Copy Hiring Link</span>
            </button>
            <a
              href="#/rider/join"
              target="_blank"
              rel="noopener noreferrer"
              className='px-3.5 py-2 rounded-2xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-xs text-emerald-400 font-bold transition flex items-center gap-1.5'
            >
        ...
        {mainTab === 'APPLICATIONS' ? (
          ...
          <div className='space-y-6'>
            ...
            <div className='flex flex-wrap items-center justify-between gap-3'>
                  ...
                  >
                    ...
                    {tab.count !== undefined && tab.count > 0 && (
                      <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${
                        appFilter === tab.id ? 'bg-rose-500 text-white' : 'bg-rose-500/20 text-rose-300'
                      }`}>
                        {tab.count}
                      </span>
                    )}
              ...
              <button
                onClick={() => fetchApplications(appFilter)}
                className='px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-400 hover:text-white font-bold transition flex items-center gap-1.5'
              >
                <IoRefresh className={loadingApps ? 'animate-spin text-emerald-400' : ''} />
                <span>Refresh</span>
              </button>
            </div>
            ...
            {loadingApps ? (
              <div className='py-20 text-center space-y-3'>
                ...
                <p className='text-xs text-slate-400'>Loading rider applications...</p>
              </div>
            ) : applications.length === 0 ? (
              <div className='bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center space-y-4 max-w-lg mx-auto'>
                <div className='w-14 h-14 bg-slate-800 rounded-2xl flex items-center justify-center mx-auto text-2xl text-slate-500'>
                  📝
                </div>
                <div>
                  <h3 className='text-base font-bold text-white'>No {appFilter.toLowerCase()} applications</h3>
                  <p className='text-xs text-slate-400 mt-1'>
                    Share the recruitment link on WhatsApp groups or with interested local drivers in Paliganj to get applications!
                  </p>
                </div>
                <button
                  onClick={handleCopyJoinLink}
                  className='inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-4 py-2.5 rounded-xl text-xs transition shadow-lg shadow-emerald-600/20'
                >
                  <FaCopy />
                  <span>Copy Link: snapit.in/#/rider/join</span>
                </button>
              </div>
            ) : (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {applications.map((app) => {
                  ...
                  return (
                    <div
                      key={app._id}
                      className='bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between space-y-4 hover:border-slate-700 transition'
                    >
                      {/* Header */}
                      <div className='space-y-2'>
                        <div className='flex items-start justify-between gap-2'>
                          <div>
                            <h3 className='font-black text-white text-base'>{app.name}</h3>
                            <p className='text-xs text-slate-400 font-mono'>+91 {app.mobile}</p>
                          </div>
                          <span
                        ...
                        <div className='bg-slate-950/70 border border-slate-800/80 rounded-xl p-3 space-y-1.5 text-xs'>
                          <div className='flex items-center justify-between text-slate-300'>
                            <span className='text-slate-400 flex items-center gap-1.5'>
                              {app.vehicleType === 'BICYCLE' ? <FaBicycle className='text-sky-400' /> : <FaMotorcycle className='text-emerald-400' />}
                              <span>Vehicle:</span>
                            </span>
                          ...
                          {app.licenseNumber && (
                            <div className='flex items-center justify-between text-slate-300'>
                              ...
                              <span className='font-mono text-slate-200'>{app.licenseNumber}</span>
                            </div>
                          )}

                          <div className='flex items-center justify-between text-slate-300'>
                            <span className='text-slate-400'>Shift:</span>
                            <span className='font-bold text-emerald-400'>{app.preferredHours}</span>
                          </div>

                          <div className='flex items-center justify-between text-slate-300'>
                            <span className='text-slate-400'>Town / Area:</span>
                            <span className='text-slate-200'>{app.area || 'Paliganj'}</span>
                          </div>
                      ...
                      <div className='flex items-center gap-2'>
                        <a
                          href={`tel:${app.mobile}`}
                          className='flex-1 py-2 px-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition'
                        >
                      ...
                      {isPending && (
                        <div className='flex items-center gap-2 pt-1'>
                          <button
                            ...
                            className='flex-1 py-2.5 px-3 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 disabled:opacity-50 text-white font-black rounded-xl text-xs flex items-center justify-center gap-1.5 transition shadow-lg shadow-emerald-600/20'
                          >
                            {isActionLoading ? (
                              ...
                              <>
                                <FaCheck size={11} />
                                <span>1-Tap Approve & Activate</span>
                              </>
                            )}
                          </button>
                          ...
                          <button
                            ...
                            className='py-2.5 px-3 bg-rose-950/50 hover:bg-rose-900/60 border border-rose-800/60 text-rose-300 rounded-xl text-xs font-bold flex items-center justify-center transition'
                      ...
                      {isRejected && (
                        <div className='bg-rose-950/40 border border-rose-500/30 rounded-xl p-2.5 text-center text-xs text-rose-300 font-medium'>
                          ❌ Application Rejected ({app.adminNotes || 'No notes'})
                        </div>
                      )}
                    </div>
                  )
