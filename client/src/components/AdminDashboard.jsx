...
const AdminDashboard = () => {
  ...
  // 2. Fetch total product count (all pages)
  ...
  // 3. Fetch rider count from DB — fetches all users with role "rider"
  ...
  // 4. Fetch all orders → compute revenue, pending, uncollected cash
  const fetchAllOrders = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // REVENUE: total from all delivered orders
        // Tries totalAmt, totalAmount, grandTotal, subTotalAmt — covers most schemas
        ...
        // UNCOLLECTED CASH: COD delivered but not settled
  ...
  const statCards = [
    {
      ...
      sub: revenue === 0 ? "No delivered orders yet" : "From all delivered orders",
      subColor: "#4ade80",
    },
    {
      label: "CATALOG",
    ...
    },
    {
      ...
      iconBg: "#3a2010",
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fb923c" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="p-4 lg:p-8 min-h-screen" style={{ background: "#0a0f1a" }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {statCards.map((card) => (
          ...
          >
            ...
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: "#6b7280", textTransform: "uppercase", marginBottom: 4 }}>
              {card.label}
            </div>
            <div style={{ fontSize: 28, fontWeight: 800, color: "#f1f5f9", letterSpacing: -0.5 }}>
              {card.value}
            </div>
            <div style={{ fontSize: 11, color: card.subColor, marginTop: 4 }}>
              {card.sub}
            </div>
          </div>
        ))}
      ...
      {/* ── UNCOLLECTED CASH BANNER (only shows when cash > 0) ── */}
      {totalCash > 0 && (
        <div
          className="mb-6 flex justify-between items-center p-5 rounded-2xl"
                  ...
                  {orders.length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {orders.slice(0, 8).map(order => (
                        ...
                        >
                          <span style={{ fontSize: 10, fontWeight: 700, color: "#4b5563", fontFamily: "monospace" }}>
                            #{(order.orderId || order._id)?.slice(-6)}
                          </span>
                          <span style={{
                            fontSize: 12, fontWeight: 700, color: "#cbd5e1",
                            flex: 1, textAlign: "center", paddingInline: 10,
