...
// Draggable and clickable map marker
...
const AddAddress = ({ close, initialCoords, initialAccuracy }) => {
  ...
  // Pre-fill user mobile if available
  useEffect(() => {
    if (user?.mobile && !watch('mobile')) {
      setValue('mobile', user.mobile)
    }
  }, [user, setValue, watch])

  // Central function to update coordinates and run reverse geocoding
  ...
  // Toggle quick delivery instruction tag
        ...
        <div className='flex justify-between items-center px-4 sm:px-6 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/80'>
          <div>
            ...
            <p className='text-[11px] text-slate-400 font-bold'>
              Live pin accurate to your doorstep • 14km Paliganj Radius
            </p>
          </div>
          <button
            onClick={close}
        ...
        <div className='overflow-y-auto p-4 sm:p-6 space-y-4 flex-1'>
          ...
          <div className='space-y-2'>
            <div className='rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-md h-56 sm:h-64 relative bg-slate-100 dark:bg-slate-900'>
              ...
              {/* Floating "Locate Me" Button (Zepto / Swiggy signature) */}
              <button
                type='button'
          ...
          {/* ── 2. ORDER FOR (SELF vs SOMEONE ELSE) ── */}
          ...
          <form className='space-y-3 pt-1' onSubmit={handleSubmit(onSubmit)}>
            ...
            {/* Auto-filled Area / Street Address */}
            <div>
            ...
            {/* City & Pincode (Auto-resolved from Pin) */}
            ...
            {/* Zepto Quick Delivery Instructions Chips */}
            <div>
              <label className='text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1.5'>
