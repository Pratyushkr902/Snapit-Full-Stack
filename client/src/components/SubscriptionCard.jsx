    ...
    const handleStatusToggle = async () => {
        ...
        try {
            const response = await Axios({
                method: 'PUT',
                url: `/api/subscription/update-status/${subscription._id}`,
    ...
    return (
        <div className="bg-white border rounded-xl p-5 shadow-sm my-3 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
                <img 
                    src={subscription.productId.image[0]} 
                    alt={subscription.productId.name} 
                    className="w-16 h-16 object-cover rounded-lg bg-gray-50"
                />
                <div>
                    <h3 className="font-semibold text-gray-800 text-lg">{subscription.productId.name}</h3>
                    <p className="text-sm text-gray-500 flex items-center gap-1 mt-0.5">
                        <FaCalendarAlt className="text-gray-400" /> Frequency: <span className="font-medium text-gray-700 capitalize">{subscription.frequency}</span>
                    </p>
                    <p className="text-sm text-gray-500 flex items-center gap-1 mt-0.5">
                        <FaTruck className="text-gray-400" /> Next Delivery: <span className="font-medium text-gray-700">{new Date(subscription.nextDeliveryDate).toLocaleDateString()}</span>
                    </p>
                </div>
            </div>

            {/* Quantity and Price Ledger */}
            <div className="flex items-center justify-between w-full md:w-auto md:gap-8 border-t md:border-t-0 pt-3 md:pt-0">
                <div className="text-left md:text-center">
                    ...
                    <span className="text-gray-800 font-semibold">{subscription.quantity} units</span>
                </div>
                <div className="text-right md:text-center">
                    <span className="text-xs text-gray-400 block uppercase font-bold tracking-wider">Total</span>
                    <span className="text-green-700 font-bold">₹{subscription.totalPrice}</span>
                </div>
            </div>
