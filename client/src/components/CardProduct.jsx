...
const CardProduct = ({data}) => {
    ...
    const getProductLabel = () => {
        ...
        if (title.includes("chicken") || title.includes("fish") || title.includes("meat")) return { text: "Fresh", color: "bg-red-600" };
        if (title.includes("organic") || title.includes("nature")) return { text: "Organic", color: "bg-emerald-600" };
        if (data?.discount > 20) return { text: "Deal", color: "bg-orange-500" };
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <div className='flex flex-col mt-1.5 gap-0.5'>
                ...
                {/* Name */}
                <div className='font-bold text-slate-800 dark:text-zinc-100 text-[10px] lg:text-sm line-clamp-2 leading-tight group-hover:text-green-700 transition-colors'>
                    {data?.name}
                </div>
                    ...
                    <div className='flex-shrink-0 w-[60px] lg:w-[90px]' onClick={(e) => e.preventDefault()}>
                        {data?.stock == 0 ? (
                            <div className='border border-red-100 bg-red-50 px-1 py-1 rounded text-center'>
                                <p className='text-red-500 text-[7px] lg:text-[9px] font-black uppercase leading-none'>Out of<br/>stock</p>
                            </div>
                        ) : (
