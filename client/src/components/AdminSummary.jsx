...
const AdminSummary = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const revenue = allOrders
          ...
          .reduce((acc, curr) => acc + (curr.totalAmt || curr.totalAmount || curr.grandTotal || 0), 0)
  ...
  const fetchRiders = async () => {
    try {
      ...
      if (response.data.success) {
        setRiderCount(response.data.data?.length ?? 0)
      }
    } catch (error) {
  ...
  const stats = [
    ...
    },
    {
      ...
      color: 'text-rose-400',
      bg: 'bg-rose-500/10',
      trend: pendingOrders > 0 ? 'Urgent' : 'Clear'
    },
    {
      label: 'Riders',
      value: riderCount,
  ...
  return (
    <div className={`${darkMode ? 'bg-[#0f172a] text-slate-200' : 'bg-slate-50 text-slate-900'} min-h-screen p-6 transition-colors duration-500`}>
        ...
        {/* Revenue Chart + Scale Operations */}
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8'>
                ...
                <div className='h-48 flex items-end gap-3 px-2'>
                    {[30, 45, 35, 60, 55, 80, 95, 70, 85, 65, 75, 90].map((h, i) => (
                        <div key={i} className='group relative flex-1'>
                            <div 
                                className={`w-full rounded-t-lg transition-all duration-1000 ease-out ${darkMode ? 'bg-blue-500/40 group-hover:bg-blue-400' : 'bg-blue-500 group-hover:bg-blue-600'}`}
            ...
            <div className={`p-8 rounded-3xl border flex flex-col items-center text-center justify-center transition-all ${darkMode ? 'bg-gradient-to-br from-green-500/20 to-blue-500/10 border-green-500/20' : 'bg-slate-900 border-slate-800'}`}>
                ...
                <p className='text-slate-400 text-sm mt-3 px-2 leading-relaxed'>
                    Real-time monitoring enabled. Ready to process batch orders for the sector.
                </p>
                <button 
